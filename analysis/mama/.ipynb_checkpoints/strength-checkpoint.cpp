{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,   5.856,10,1.067e-09,2.671e-06);
	ifstream strengthfile("strength.nrm");
	float strength[62],strengtherr[62],energyerr[62];
	float energy[611],trans[611];
	int i = 0;
   float a0 =  -0.8671;
   float a1 =   0.1037;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-61] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(62,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.071,8.013e-07,"^{233}U");
	c1->Update();
