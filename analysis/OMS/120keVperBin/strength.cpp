{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,   5.849,10,3.122e-09,6.045e-06);
	ifstream strengthfile("strength.nrm");
	float strength[54],strengtherr[54],energyerr[54];
	float energy[531],trans[531];
	int i = 0;
   float a0 =  -0.8914;
   float a1 =   0.1200;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-53] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(54,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.070,1.814e-06,"^{xx}Yy");
	c1->Update();
