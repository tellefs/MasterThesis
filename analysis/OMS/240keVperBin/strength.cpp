{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,   5.969,10,2.588e-09,1.855e-06);
	ifstream strengthfile("strength.nrm");
	float strength[28],strengtherr[28],energyerr[28];
	float energy[271],trans[271];
	int i = 0;
   float a0 =  -0.7714;
   float a1 =   0.2400;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-27] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(28,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.094,5.564e-07,"^{xx}Yy");
	c1->Update();
