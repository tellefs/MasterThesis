{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,   5.799,10,1.930e-09,3.745e-06);
	ifstream strengthfile("strength.nrm");
	float strength[35],strengtherr[35],energyerr[35];
	float energy[341],trans[341];
	int i = 0;
   float a0 =  -0.8064;
   float a1 =   0.1850;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-34] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(35,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.060,1.124e-06,"^{xx}Yy");
	c1->Update();
