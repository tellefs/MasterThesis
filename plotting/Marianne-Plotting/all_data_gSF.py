...
with open("../experimental_data/187Re_photo_load1.json") as K:
...
plt.errorbar(energy_d, strength_d, yerr=strength_err_d[:-1], x_err=None, fmt="*", color="#FF5733", label="(a,d)188Re")
plt.plot(energy_trans_d[:80], trans_d[:80], color="#FFBD33")

plt.errorbar(energy_t, strength_t, yerr=strength_err_t[:-1], x_err=None, fmt="*", color="#499F20", label="(a,t)187Re")
plt.plot(energy_trans_t[:80], trans_t[:80], color="#75FF33")

plt.title("gamma-ray SF for both (a,d) and (a,t) with extrapolations.")
plt.ylabel("gSF [1/MeV^3]")
